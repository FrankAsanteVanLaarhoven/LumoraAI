...
export async function POST(req: NextRequest) {
  ...
  if (body.authorized !== true) {
    await audit('crawl', url, 'refused', 'missing authorization attestation');
  ...
  const result = await crawlSite(url, { depth, limit, sameOrigin, render });
  await audit('crawl', url, 'ok', `${result.visited} pages, ${result.skipped.length} skipped`);
